...
#### Team Plot
****
* Question: What parks did people bike to using bike station on the fourth of July?

```{r,echo=FALSE, warning = FALSE}

#What parks did people bike to using bike station on the fourth of July?


parques <- trip %>% filter(str_detect(`End station`, "Park$"))
parques1 <- parques %>% filter(date == "2017-07-04")

  
autoplot(map.latlon) + 
  geom_segment(data=parques1, 
               aes(x=start_longitude, y=start_latitude,
                   xend=end_longitude, yend=end_latitude), 
               color = 'blue', size = .5,
               arrow = arrow(length = unit(0.2, "cm"))) +  geom_point(data=parques1, 
               aes(x=end_longitude, y=end_latitude), 
               color = 'red', size = 3)+
  labs(x='Longitude', y='Latitude') +
  ggtitle('Fourth of July Rides to Parks')
...
* Here you can see the plot of Ryan, showing all of the bike trips from bike stations to parks on the fourth of July, with added markers for each park. This plot is interesting because it shows the parks most popular on the 4th of July, maybe because they had fireworks or some other event going on. In addition, the plot shows the distribution of stations that people left from to get to these parks.
...
****
```{r,echo=FALSE}
ggplot(data = onehour, mapping = aes(x = minute)) + geom_density()+ ggtitle("Use Duration of NiceRide Bikes") + ylab("Density")+ xlab("Minutes")
count(onehour) 
...
#### Arie's Section
****
* Question: What is the average lattitude traveled depending on the account type?
```{r, echo=FALSE}

merged22 <- merged %>% mutate(long_diff = abs(start_longitude - end_longitude), latt_diff = abs(start_latitude-end_latitude))

gg1 <- merged22 %>% select(c(`Start station`, `End station`, long_diff, latt_diff, `Total duration (Seconds)`, `Account type`)) %>%
  filter(long_diff > 0.001, latt_diff > 0.001)%>%
  group_by(`Start station`, `End station`, `Total duration (Seconds)`, `Account type`)%>%
  summarise(avg_long = mean(long_diff), avg_latt = mean(latt_diff))

ggplot(data = gg1, mapping = aes(x = avg_latt))+
    geom_density()+ facet_wrap(~`Account type`)+ ggtitle("Distribution of Average Latitude")+xlab("Average Latitude")+ylab("Count")

```

* Findings: From my graph I found that there was not much of a difference in the distribution of latitude traveled depedning on the account type. The trend of each account is pretty much the same. However, the distribution of memembers is more. 
* I used `mutate()` to create new columns that show the total distance for latitude and longitude. Then I created a new varible and selected the data that I wanted to focus on. Then I used `filter()` the longitude and latitude that were greater than .001. Then I used `group_by()` and `summarise()` to find the average distance for latitude and longitude. After I created a `ggplot()` with the new data variable. I used `geom_denisty()` and `facet_wrap()` to compare the distribution by account type.
...
Main quesitno: What parks did people bike to using bike station on the fourth of July?

Did people bike to one particlar park or multiple ones?

which park did most people bike to?
...
* I worked on joining the datasets and finding out how to convert the dates to a datetime. I also filtered by date and by statiion ending in the work Park in order to find the trips to parks. I then used geom_segment to illustrate the bike stations people left from to get to the parks. Afterwards, I scaled the graph to focus on the trips to the popular parks in Minneapolis. 
* findings: Most people biked to Minnehaha park, followed by Gateway park and Logan park. People biked to 11 parks in total. 

**** 
#### Who did what?

* David - Team jokes and awesome wheelchair video, individual sectio, rMarkdown edits, mananged gitKraken errors, ploted points on team map, ran some openmap stuff with Ryan.
* Arie- Individual section
* Anderson- Individual section and findings, Seperated and tidied the dataset
* Ethan- ind section and findings, merged and tidied/sorted the two datasets
* Ryan- indivual section. Helped create team plot. filtered, plotted, and string detected